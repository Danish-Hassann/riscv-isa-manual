...
. *Machine Security Configuration (mseccfg)* is a new RW Machine mode CSR, used for configuring various security mechanisms present on the hart, and only accessible to Machine mode. [#norm:mseccfg_address]#It is 64 bits wide, and is at address *0x747 on RV64* and *0x747 (low 32bits), 0x757 (high 32bits) on RV32*.# [#norm:mseccfg_WARL]#All mseccfg fields defined on this extension are WARL, and the remaining bits are reserved for future standard use and should always read zero.# [#norm:mseccfg_reset_value]#The reset value of mseccfg is implementation-specific, otherwise if backwards compatibility is a requirement it should reset to zero on hard reset.#
...
[#norm:mseccfg-MMWP_sticky_bit]#. On ``mseccfg`` we introduce a field in bit 1 called *Machine-Mode Allowlist Policy (mseccfg.MMWP)*. This is a sticky bit, meaning that once set it cannot be unset until a *PMP reset*.# [#norm:mseccfg-MMWP_op]#When set it changes the default PMP policy for M-mode when accessing memory regions that don’t have a matching PMP rule, to *denied* instead of *ignored*.#

[#norm:mseccfg-MML_sticky_bit]#. On ``mseccfg`` we introduce a field in bit 0 called *Machine Mode Lockdown (mseccfg.MML)*. This is a sticky bit, meaning that once set it cannot be unset until a *PMP reset*.# When ``mseccfg.MML`` is set the system's behavior changes in the following way:

.. [#norm:mseccfg-MML_op]#The meaning of ``pmpcfg.L`` changes: Instead of marking a rule as *locked* and *enforced* in all modes, it now marks a rule as *M-mode-only* when set and *S/U-mode-only* when unset.# [#norm:smepmp_shared_region_encoding]#The formerly reserved encoding of ``pmpcfg.RW=01``, and the encoding ``pmpcfg.LRWX=1111``, now encode a *Shared-Region*.#
+
[[norm:mmode_only_rule]]
...
+
[#norm:smepmp_shared_data_region_rule]#* A _Shared-Region_ rule where ``pmpcfg.L`` is not set can be used for sharing data between M-mode and S/U-mode, so is not executable.# [#norm:smepmp_shared_data_region_rw_access]#M-mode has read/write access to that region, and S/U-mode has read access if ``pmpcfg.X`` is not set, or read/write access if ``pmpcfg.X`` is set.#
+
[#norm:smepmp_shared_code_region_rule]#* A _Shared-Region_ rule where ``pmpcfg.L`` is set can be used for sharing code between M-mode and S/U-mode, so is not writable.# [#norm:smepmp_shared_code_region_rx_access]#Both M-mode and S/U-mode have execute access on the region, and M-mode also has read access if ``pmpcfg.X`` is set.# [#norm:smepmp_shared_code_region_rule_locked]#The rule remains *locked* so that any further modifications to its associated configuration or address registers are ignored until a *PMP reset*, unless ``mseccfg.RLB`` is set.#
+
[[norm:smepmp_shared_region_readonly_access]]
...
[[norm:smepmp_mmode_exec_permission]]
...
+
[#norm:smepmp_shared_region_no_exec_priv_rule]#Also when ``mseccfg.MML`` is set, according to 4b it’s not possible to add a _Shared-Region_ rule with executable privileges.# [#norm:smepmp_shared_region_exec_priv_rule_on_RLB_set]#So RLB can be set temporarily during the boot process to register such regions.# [#norm:smepmp_shared_region_exec_priv_rule_on_pmp_reset]#Note that it’s still possible to register executable _Shared-Region_ rules using initial register settings (that may include ``mseccfg.MML`` being set and the rule being set on PMP registers) on *PMP reset*, without using RLB.#
+
[WARNING]
====
...
[IMPORTANT]
====
[[norm:pmp_locked_rules_priority]]
